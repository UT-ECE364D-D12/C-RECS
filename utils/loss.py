...
class EncoderCriterion(Criterion):
    def __init__(self, expander: nn.Module, classifier: nn.Module, loss_weights: Dict[str, float] = {}, triplet_margin: float = 1.0, focal_gamma: float = 0.0, vicreg_gamma: float = 1.0, max_rank: int = 50) -> None:
        super().__init__()
    ...
    def forward(self, anchor: Tuple[Tensor, Tensor], positive: Tuple[Tensor, Tensor], negative: Tuple[Tensor, Tensor]) -> Dict[str, Tensor]:
        ...
        id_loss = (self._get_focal_loss(prediction_anchor_logits, anchor_ids) + self._get_focal_loss(prediction_positive_logits, positive_ids) + self._get_focal_loss(prediction_negative_logits, negative_ids)) / 3
