...
def main(data_root: Path, max_length: int = 64, train_size: float = 0.8, val_size: float = 0.1) -> None:
    ...
    for model_name, batch_size in zip(MODEL_NAMES, BATCH_SIZES):
        ...
        model_data = simulate(model, tokenizer, dataloader, max_length=max_length).rename(columns={"text": "request"})
    ...
    data.to_csv(data_root / f"requests.csv", index=False)

    # Split the requests into train, validation, and test sets
    data = data.groupby("item_id").agg({"item_title": "first", "request": list}).reset_index()
    ...
    def split_request(row: pd.Series) -> pd.Series:
        train_req, test_req = train_test_split(row["request"], test_size=test_size)
        train_req, val_req = train_test_split(train_req, test_size=val_size)

        return pd.Series([train_req, val_req, test_req])

    data[["train_requests", "val_requests", "test_requests"]] = data.apply(split_request, axis=1)

    train_data = data[["item_id", "item_title", "train_requests"]].rename(columns={"train_requests": "request"})
    val_data = data[["item_id", "item_title", "val_requests"]].rename(columns={"val_requests": "request"})
    test_data = data[["item_id", "item_title", "test_requests"]].rename(columns={"test_requests": "request"})

    # Flatten the requests
    train_data = train_data.explode("request").reset_index(drop=True)
    val_data = val_data.explode("request").reset_index(drop=True)
    test_data = test_data.explode("request").reset_index(drop=True)

    train_data.to_csv(data_root / f"train_requests.csv", index=False)
    val_data.to_csv(data_root / f"val_requests.csv", index=False)
    test_data.to_csv(data_root / f"test_requests.csv", index=False)
