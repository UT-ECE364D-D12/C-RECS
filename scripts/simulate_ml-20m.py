...
def main(data_root: Path, train_size: float = 0.8, val_size: float = 0.1) -> None:
    ...
    for model_name, batch_size in zip(MODEL_NAMES, BATCH_SIZES):
        ...
        dataset = SimulatorDataset(movies, language_tokenizer, REQUEST_PROMPTS)
    ...
    data.to_csv(data_root / "requests.csv", index=False)
    ...
    data[["train_request", "val_request", "test_request"]] = data.apply(split_request, axis=1)

    train_data = data[["item_id", "item_title", "train_request"]].rename(columns={"train_request": "request"})
    val_data = data[["item_id", "item_title", "val_request"]].rename(columns={"val_request": "request"})
    test_data = data[["item_id", "item_title", "test_request"]].rename(columns={"test_request": "request"})
    ...
    train_data.to_csv(data_root / "train_requests.csv", index=False)
    val_data.to_csv(data_root / "val_requests.csv", index=False)
    test_data.to_csv(data_root / "test_requests.csv", index=False)
